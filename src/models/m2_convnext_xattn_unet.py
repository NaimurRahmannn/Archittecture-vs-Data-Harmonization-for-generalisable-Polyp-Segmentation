...
class CrossAttentionFusion(nn.Module):
    def __init__(self, decoder_ch, encoder_ch, attn_dim=64, max_tokens=1024):
        ...
        self.out_proj = nn.Conv2d(self.attn_dim, decoder_ch, kernel_size=1, bias=False)

        self.q_norm = nn.LayerNorm(self.attn_dim)
        self.k_norm = nn.LayerNorm(self.attn_dim)
        self.v_norm = nn.LayerNorm(self.attn_dim)
    ...
    def forward(self, dec_feat, enc_feat):
        ...
        if scale_d > 1 or scale_e > 1:
            out = F.interpolate(out, size=dec_feat.shape[-2:], mode="bilinear", align_corners=False)

        return dec_feat + out


class DecodeBlockXAttn(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch, attn_dim=64, max_tokens=1024):
        super().__init__()
        self.up = nn.ConvTranspose2d(in_ch, out_ch, kernel_size=2, stride=2)
        self.fusion = CrossAttentionFusion(out_ch, skip_ch, attn_dim=attn_dim, max_tokens=max_tokens)
...
class M2ConvNeXtXAttnUNet(nn.Module):
    def __init__(self, encoder_name="convnext_tiny", pretrained=True, attn_dim=64, max_tokens=1024):
        ...
        try:
            ...
            raise ImportError(
                "timm is required for M2ConvNeXtXAttnUNet. Install it with: pip install timm"
            ) from exc
        ...
        self.dec4 = DecodeBlockXAttn(c4, c4, c3, attn_dim=attn_dim, max_tokens=max_tokens)
        self.dec3 = DecodeBlockXAttn(c3, c3, c2, attn_dim=attn_dim, max_tokens=max_tokens)
        self.dec2 = DecodeBlockXAttn(c2, c2, c1, attn_dim=attn_dim, max_tokens=max_tokens)
        self.dec1 = DecodeBlockXAttn(c1, c1, 64, attn_dim=attn_dim, max_tokens=max_tokens)
